...
/**
 * @file
 * Shared helper functions for stuar.tc's Drupal DevTools scripts.
 *
 * Adapted from the `.devtools/` pattern used across Stuart's own Drupal
 * extension repos (field_tokens, custom_formatters, imagefield_tokens —
 * itself in the spirit of AlexSkrypnyk/drupal_extension_scaffold), but for
 * a full site checkout rather than a bare extension: no `build/` directory,
 * no `composer create-project` scaffolding step — `drupal/` already IS the
 * site codebase, so `assemble` is just `composer install` and `provision`
 * runs `drush tome:install` against the committed config/content instead of
 * a vanilla `site-install`.
 *
 ...
 * and executed in order. Each script inherits the parent's cwd and
 * environment. A non-zero exit from any script aborts the parent. A
 * missing directory or empty match set returns silently — this is the
 * post-assemble / post-provision / post-start / pre-stop extension point.
 */
...
// Never run the real quit() during tests; mirrors the equivalent function in
// the extension-scaffold-derived .devtools/ scripts across Stuart's other
// Drupal repos.
// @codeCoverageIgnoreStart
if (!function_exists('StuartcDevTools\quit') && !class_exists('PHPUnit\\Framework\\TestCase')) {
