...
import DevGridTools from '~/components/DevGridTools.vue'
...
/** Mount DevGridTools directly, attached to document.body so its
 *  non-teleported sections (and its own Teleport content) are both
 *  findable via document.querySelector, matching real usage inside
 *  DevGrid's console modal. */
async function mount() {
  const w = await mountSuspended(DevGridTools, {
...
async function activateMeasure() {
  const startBtn = [...document.querySelectorAll<HTMLElement>('button')]
...
afterEach(async () => {
  ...
  await nextTick()
  await nextTick()
  document.querySelectorAll('[data-dev-console],[data-grid-overlay],.target-el,.el-b,.el-c')
...
describe('DevGridTools startup', () => {
  ...
  it('loads grid overlay from localStorage (grid:8)', async () => {
    localStorage.setItem(STORAGE_KEY, JSON.stringify({ grid: '8' }))
    const w = await mount()
  ...
  it('writes state to localStorage when prefs change', async () => {
    const w = await mount()
    const btn4 = [...document.querySelectorAll<HTMLElement>('button')]
      .find(b => b.textContent?.trim() === '4px')
    btn4?.click(); await nextTick()
    const saved = JSON.parse(localStorage.getItem(STORAGE_KEY) ?? '{}')
    expect(saved).toHaveProperty('grid', '4')
    w.unmount()
  })
...
describe('overlay toggles', () => {
  it('grid toggle: off → 4px → 8px → off', async () => {
    ...
    const btn8 = [...document.querySelectorAll<HTMLElement>('button')]
      .find(b => b.textContent?.trim() === '8px')
    btn8?.click(); await nextTick()
    expect(document.body.querySelector('[data-grid-overlay]')).not.toBeNull()

    const btnOff = [...document.querySelectorAll<HTMLElement>('button')]
      .find(b => b.textContent?.trim() === 'Off' && b.closest('section')?.textContent?.includes('DEV OVERLAY'))
  ...
  it('columns toggle shows/hides column guides', async () => {
    const w = await mount()
    const colRow = [...document.querySelectorAll<HTMLElement>('[style*="space-between"]')]
  ...
  it('outlines toggle adds/removes body class', async () => {
    const w = await mount()
    const outRow = [...document.querySelectorAll<HTMLElement>('[style*="space-between"]')]
  ...
  it('color scheme buttons mutate appConfig.ui.colors.primary', async () => {
    const w = await mount()
    const electricBtn = [...document.querySelectorAll<HTMLElement>('button')]
  ...
  it('module list "Show filtered modules" toggle flips between Off and On', async () => {
    const w = await mount()
    const label = [...document.querySelectorAll<HTMLElement>('h3')]
...
describe('measure mode', () => {
  it('Start button enters measure mode and requests the console close', async () => {
    const w = await mount()
    const blocker = await activateMeasure()
    await nextTick()
    expect(blocker).not.toBeNull()
    expect(w.emitted('update:showConsole')?.at(-1)).toEqual([false])
    w.unmount()
  })
  ...
  it('measure button shows "Active" while measuring', async () => {
    ...
    await nextTick()
    const activeBtn = [...document.querySelectorAll<HTMLElement>('button')]
